...
public class SkullBlock {
	...
	private static final Map<Integer, Material> skullTypeList = new HashMap<>();
	private static final Map<Integer, BlockFace> skullRotationList = new HashMap<>();

	static {
		skullTypeList.put(0, Material.SKELETON_SKULL);
		skullTypeList.put(1, Material.WITHER_SKELETON_SKULL);
		skullTypeList.put(2, Material.ZOMBIE_HEAD);
		skullTypeList.put(3, Material.PLAYER_HEAD);
		skullTypeList.put(4, Material.CREEPER_HEAD);
	}
	...
	public boolean set(Block block) {
		...
		skull.setType(skullType);
		((Rotatable) skull.getBlockData()).setRotation(skullRotation);
	//	skull.setRawData((byte) skullStanding);
		// Texture update
		if (skullTextureValue != null) {
			setSkullWithNonPlayerProfile(skullTextureValue, skullTextureSignature, skullOwnerUUID, skullOwnerName,
					skull);
		}
	...
	public boolean prep(Map<String, Tag> tileData, BlockFace rotation) {
		...
		try {
			...
			if (skullType == Material.PLAYER_HEAD && tileData.containsKey("Owner")) {
	...
	// @SuppressWarnings("deprecation")
	public static void setSkullWithNonPlayerProfile(String textureValue, String textureSignature, String ownerUUID,
			String ownerName, Skull skull) {
		...
		// skull.getWorld().refreshChunk(skull.getChunk().getX(),
		// skull.getChunk().getZ());
	}
	...
	public static GameProfile getNonPlayerProfile(String textureValue, String ownerUUID, String ownerName) {
		// Create a new GameProfile with .schematic informations or with fake
		// informations
	...
	// Credits: dori99xd
	public static GameProfile getPlayerProfile(String textureValue, String textureSignature, String ownerUUID,
			String ownerName) {
		// Create a new GameProfile with .schematic informations or with fake
		// informations
		GameProfile newSkinProfile = new GameProfile(ownerUUID == null ? UUID.randomUUID() : UUID.fromString(ownerUUID),
