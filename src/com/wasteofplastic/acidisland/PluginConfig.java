...
public class PluginConfig {
	 ...
	 * 
	 * @param plugin
	 *            - ASkyBlock plugin object - askyblock
	...
	/*	if (ASkyBlock.isOnePointThirteen()) {
			plugin.getLogger().info("Version 1.13 detected! Please wait...");
			if (plugin.getConfig().get("onePointThirteen") == null) {
				if (!UpdateOnePointThirteen.updateOnePointThirteen(plugin)) {
					return false;
				}

			}
		}*/
		// The order in this file should match the order in config.yml so that it is
		// easy to check that everything is covered
		// ********************** Island settings **************************
		...
		if (Settings.islandProtectionRange <= 0) {
			...
			plugin.getLogger()
					.warning("equal the island distance and then you should allow all island protection flags");
			plugin.getLogger().warning(
					"in config.yml. Protection range will be set to island distance (" + Settings.islandDistance + ")");
			Settings.islandProtectionRange = Settings.islandDistance;
		}
		...
		// xoffset and zoffset are not public and only used for IslandWorld
		// compatibility
		...
		Settings.islandStartX = Math.round((double) x / Settings.islandDistance) * Settings.islandDistance
				+ Settings.islandXOffset;
		Settings.islandStartZ = Math.round((double) z / Settings.islandDistance) * Settings.islandDistance
		 ...
		 * Settings.maxTeamSizeVIP2 = plugin.getConfig().getInt("island.vip2teamsize",
		 * 0); if (Settings.maxTeamSizeVIP > 0 || Settings.maxTeamSizeVIP2 > 0) {
		 * plugin.getLogger().warning(Settings.PERMPREFIX + "team.vip and " +
		 * Settings.PERMPREFIX + "team.vip2 are deprecated!");
		 * plugin.getLogger().warning("Use permission " + Settings.PERMPREFIX +
		 * "team.maxsize.<number> instead."); }
		 */
				...
				try {
					if (amountdata.length == 2 && amountdata[0].endsWith("SPAWN_EGG")) {
						try {
							...
							List<String> SpawnEgglist = new ArrayList<>(Arrays.asList("BAT_SPAWN_EGG",
									"BLAZE_SPAWN_EGG", "CAVE_SPIDER_SPAWN_EGG", "CHICKEN_SPAWN_EGG", "COD_SPAWN_EGG",
									"COW_SPAWN_EGG", "CREEPER_SPAWN_EGG", "DOLPHIN_SPAWN_EGG", "DONKEY_SPAWN_EGG",
									"DROWNED_SPAWN_EGG", "ELDER_GUARDIAN_SPAWN_EGG", "ENDERMAN_SPAWN_EGG",
									"ENDERMITE_SPAWN_EGG", "EVOKER_SPAWN_EGG", "GHAST_SPAWN_EGG", "GUARDIAN_SPAWN_EGG",
									"HORSE_SPAWN_EGG", "HUSK_SPAWN_EGG", "LLAMA_SPAWN_EGG", "MAGMA_CUBE_SPAWN_EGG",
									"MOOSHROOM_SPAWN_EGG", "MULE_SPAWN_EGG", "OCELOT_SPAWN_EGG", "PARROT_SPAWN_EGG",
									"PHANTOM_SPAWN_EGG", "PIG_SPAWN_EGG", "POLAR_BEAR_SPAWN_EGG",
									"PUFFERFISH_SPAWN_EGG", "RABBIT_SPAWN_EGG", "SALMON_SPAWN_EGG", "SHEEP_SPAWN_EGG",
									"SHULKER_SPAWN_EGG", "SILVERFISH_SPAWN_EGG", "SKELETON_HORSE_SPAWN_EGG",
									"SKELETON_SPAWN_EGG", "SLIME_SPAWN_EGG", "SPIDER_SPAWN_EGG", "SQUID_SPAWN_EGG",
									"STRAY_SPAWN_EGG", "TROPICAL_FISH_SPAWN_EGG", "TURTLE_SPAWN_EGG", "VEX_SPAWN_EGG",
									"VILLAGER_SPAWN_EGG", "VINDICATOR_SPAWN_EGG", "WITCH_SPAWN_EGG", "WOLF_SPAWN_EGG",
									"ZOMBIE_HORSE_SPAWN_EGG", "ZOMBIE_PIGMAN_SPAWN_EGG", "ZOMBIE_SPAWN_EGG",
							...
							/*
							 * for (Material type : Material.values()) { if (type.is) {
						...
						if (amountdata.length == 2) {
							 ...
							 * else if (amountdata.length == 3) { tempChest[i] = new ItemStack(mat,
							 * Integer.parseInt(amountdata[2]), Short.parseShort(amountdata[1])); }
							 */
					}
		...
		// Clean up blocks around edges when deleting islands - this is a hidden setting
		// not in the config.yml
		...
		if (islandFile.exists()) {
			...
			try {
				...
				if (!islandYaml.contains(Settings.worldName)) {
					...
					plugin.getLogger()
							.severe("If you are resetting and changing world, delete island.yml and restart.");
					plugin.getLogger().severe("");
					plugin.getLogger().severe(
							"+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+");
					return false;
		...
		if (!availableLocales.containsKey(Settings.defaultLanguage)) {
			plugin.getLogger().severe(
					"'" + Settings.defaultLanguage + ".yml' not found in /locale folder. Using /locale/en-US.yml");
		...
		// How long a player has to wait after deactivating PVP until they can activate
		// PVP again
			...
			for (EntityType type : EntityType.values()) {
				if (mobName.toUpperCase().equals(type.toString())) {
					try {
						...
						plugin.getLogger()
								.severe("Error in config.yml, mobwhitelist value '" + mobName + "' is invalid.");
					}
					mobFound = true;
					break;
				}
			...
			for (String entity : entityLimits.getKeys(false)) {
				...
				if (Material.getMaterial(entity.toUpperCase()) != null && limit > -1) {
					...
					if (entity.equalsIgnoreCase("REDSTONE_COMPARATOR")) {
						...
						Settings.limitedBlocks.put("REDSTONE_COMPARATOR", limit);
					} else if (entity.equalsIgnoreCase("BANNER")) {
						// To simplify banners, the banner is allowed and automatically made wall and
						// standing banner
						...
						// To simplify signs, the sign is allowed and automatically made wall and
						// standing signs
		...
		Settings.removeCompleteOntimeChallenges = plugin.getConfig()
				.getBoolean("general.removecompleteonetimechallenges", false);
		...
		Settings.chatChallengeLevelPrefix = plugin.getConfig().getString("general.chatchallanegelevelprefix",
				"{ISLAND_CHALLENGE_LEVEL}");
		...
		if (Settings.useMagicCobbleGen) {
			Settings.magicCobbleGenOnlyAtSpawn = plugin.getConfig().getBoolean("general.magiccobblegenonlyatspawn",
					false);
			if (plugin.getConfig().isSet("general.magiccobblegenchances")) {
				// plugin.getLogger().info("DEBUG: magic cobble gen enabled and chances section
				// found");
				...
				for (String level : plugin.getConfig().getConfigurationSection("general.magiccobblegenchances")
						.getKeys(false)) {
					long levelLong = 0;
					try {
						...
						for (String block : plugin.getConfig()
								.getConfigurationSection("general.magiccobblegenchances." + level).getKeys(false)) {
							double chance = plugin.getConfig()
									.getDouble("general.magiccobblegenchances." + level + "." + block, 0D);
							if (chance > 0 && Material.getMaterial(block) != null
									&& Material.getMaterial(block).isBlock()) {
								// Store the cumulative chance in the treemap. It does not need to add up to
								// 100%
								chanceTotal += chance;
						...
						plugin.getLogger().severe("Unknown level '" + level
		...
		// plugin.getLogger().info("DEBUG: config ver length " +
		...
		// plugin.getLogger().info("DEBUG: version length " +
		...
		// *** Non-Public Settings - these are "secret" settings that may not be used
		// anymore
		...
		// ************ Protection Settings ****************
		// Default settings hashmaps - make sure this is kept up to date with new
		// settings
		...
		for (String setting : protectionWorld.getKeys(false)) {
			try {
				...
				plugin.getLogger().severe(
						"Unknown setting in config.yml:protection.world " + setting.toUpperCase() + " skipping...");
			}
		}
		// Establish defaults if they are missing in the config file.
		for (SettingsFlag flag : SettingsFlag.values()) {
			if (!Settings.defaultWorldSettings.containsKey(flag)) {
				plugin.getLogger().warning("config.yml:protection.world." + flag.name()
						+ " is missing. You should add it to the config file. Setting to false by default");
		...
		for (String setting : protectionIsland.getKeys(false)) {
			try {
				...
				plugin.getLogger()
						.severe("Unknown setting in config.yml:island.world " + setting.toUpperCase() + " skipping...");
			}
		}
		// ******************** Biome Settings *********************
		...
		if (blockValuesConfig.isSet("limits")) {
			for (String material : blockValuesConfig.getConfigurationSection("limits").getKeys(false)) {
				try {
					...
					if (DEBUG) {
						plugin.getLogger().info("Maximum number of " + materialData + " will be "
								+ Settings.blockLimits.get(materialData));
					}
				} catch (Exception e) {
					plugin.getLogger().warning(
							"Unknown material (" + material + ") in blockvalues.yml Limits section. Skipping...");
				}
			}
		}
		...
		if (blockValuesConfig.isSet("blocks")) {
			for (String material : blockValuesConfig.getConfigurationSection("blocks").getKeys(false)) {
				try {
					...
					if (split.length > 1) {
						plugin.getLogger()
								.warning("§cThe numeric IDs are no longer supported! Use Material names instead.");
					...
					if (DEBUG) {
						plugin.getLogger()
								.info(materialData.toString() + " value " + Settings.blockValues.get(materialData));
					}
				} catch (Exception e) {
					// e.printStackTrace();
					plugin.getLogger().warning(
							"Unknown material (" + material + ") in blockvalues.yml blocks section. Skipping...");
				}
			}
		} else {
